    ...
    if (!empty($element['#value'])) {
      ...
      if (is_array($element['#value'])) {
        ...
        foreach ($input_values as $input) {
          ...
          // If the value in parentheses is not numeric, treat it as keyword text.
            ...
            if ($autocreate) {
              if ($match !== NULL) {
                ...
                if (empty($valid_ids)) {
                  ...
                  $term_entity = \Drupal\taxonomy\Entity\Term::load(reset($valid_ids));
                  if ($term_entity !== NULL) {
                    $term_label = $term_entity->getName();
                    $term_label = str_replace("\"", "", $term_label);
                    $input_label = str_replace(' ('.$match.')', '', $input);
                    $input_label =  str_replace("\"", "", $input_label);
                    if ($term_label != $input_label) {
                      $match = NULL;
                    }
                  }
                }
