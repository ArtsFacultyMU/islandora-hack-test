...
class GlossaryHelper {
  ...
  public function glossaryGetter($source_value, array $glossary_az_grouping) {
    ...
    while (!empty($source_value) and $this->isSpecial(mb_substr(trim($source_value), 0, 1))) {
      $source_value = mb_substr(trim($source_value), 1);
    } 
    // islandora-muni end
    ...
    // islandora-muni begin
    if (str_starts_with(mb_strtoupper($source_value), 'CH')) {
      $first_letter = 'CH';
    }
    // islandora-muni end
