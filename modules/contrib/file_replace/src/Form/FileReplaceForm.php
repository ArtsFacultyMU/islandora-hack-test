...
class FileReplaceForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    dpm('original: '.$file_uri);
    dpm('replace: '.$replacement->getFileUri());

    // islandora-muni begin
    if ($replacement->getFilename() != $file->getFilename()) {
      $this->messenger()->addError(t('File '.$file->getFilename().' was not replaced with file '.$replacement->getFilename().'  because the names do not match. Please use the "Remove" button.'));
      return;
    }
